...
$queryRating = mysqli_query($link, "Select * FROM Rating WHERE RatingID=".($_GET['RatingID'])." ORDER BY CreateDate DESC");
?>
...
<div id="content">
	<div id="colOne">
        <h2> Rediger kommentar</h2>
            <?php
				while($rowRating = mysqli_fetch_assoc($queryRating)) //En while der kører alle rækker igennem
					{
						echo "<div id=\"opret\">\n";
							echo "<form method=\"post\" action=\"../food/DB/UpdateRating_DB.php\">\n";
								echo "<p>\n";
									echo "<label><h3>Indhold</h3></label><br />\n";
           							echo "<input name=\"RatingID\" type=\"hidden\" value=\"".$_GET['RatingID']."\" />\n";
           							echo "<input name=\"OpskriftID\" type=\"hidden\" value=\"".$_GET['OpskriftID']."\" />\n";
									echo "<textarea name=\"RatingContent\" cols=\"50\" rows=\"15\">".$rowRating['RatingContent']."</textarea><br /> <br />\n";
									echo "<p>";
									echo "Stjerner: <br/>";
									echo "<label>";
