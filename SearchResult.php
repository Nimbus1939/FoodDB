...
$link = OpenDB();
$Qresult = "Select * FROM Opskrift Where Overskrift LIKE '%".$_POST['textfield1']."%' ORDER BY OpskriftID DESC";
...
$result = mysqli_query($link, $Qresult);
    ...
    <div id="colOne">
        ...
        <ul>
            <?php
				if ($numResult >0)
				{
					while($row = mysqli_fetch_assoc($result)) //En while der kører alle rækker igennem
					{
						echo "<LI><A href=\"ShowOpskrift.php?OpskriftID=".$row['OpskriftID']."\">".$row['Overskrift']."</a></LI>\n";
					}
				}
				else
				{
					echo "<LI>Ingen resultater matchede din søgning efter: ".$_POST['textfield1']."</LI>";
				}
			?>
            </ul>
            <p>Fandt du ikke det du søgte? Prøv den <a href="AdvancedSearch.php">Avancerede søgning</a></p><br />
    </div>
	<div id="colTwo">
		<ul>
			<li>
