...
$QqueryKategori = "Select * FROM Kategori WHERE KategoriID=".($_GET['KategoriID'])." ORDER BY CreateDate DESC";
...
<div id="content">
	<div id="colOne">
        <h2> Rediger kategori</h2>
            <p> På denne side kan du redigere en kategori</p>
            <?php
				while($rowKategori = mysqli_fetch_assoc($queryKategori)) //En while der kører alle rækker igennem
					{
						echo "<div id=\"opret\">\n";
							echo "<form method=\"post\" action=\"../food/DB/UpdateKategori_DB.php\">\n";
								echo "<p>\n";
									echo "<label><h3>Navn</h3></label><br />\n";
									echo "<input name=\"KategoriName\" type=\"text\" size=\"50\" maxlength=\"50\" value=\"".$rowKategori['KategoriName']."\"/> <br /><br />\n";
									echo "<label><h3>Indhold</h3></label><br />\n";
           							echo "<input name=\"KategoriID\" type=\"hidden\" value=\"".$_GET['KategoriID']."\" />\n";
									echo "<textarea name=\"Content\" cols=\"50\" rows=\"15\">".$rowKategori['Content']."</textarea><br /> <br />\n";
