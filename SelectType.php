...
<div id="content">
	<div id="colOne">
        <h2> Rediger type</h2>
            <p>På denne side kan du vælge en type som skal redigeres.</p>
            <p>Vælg en type herunder, for at redigere den</p>
			<?php
			echo"<ul>";
			$queryType = mysqli_query($link, "Select * FROM Type ORDER BY TypeID DESC");
			while($rowType = mysqli_fetch_assoc($queryType)) //En while der kører alle rækker igennem
			{
			echo "<LI><A href=\"../food/EditType.php?TypeID=".$rowType['TypeID']."\">".$rowType['TypeName']."</a></LI>\n";
			}
			echo"</ul>";
			?>
	</div>
