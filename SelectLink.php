...
$Qlinks = "Select * FROM Links ORDER BY LinkID DESC";
...
$links = mysqli_query($link, $Qlinks);
?>
...
<script type="text/javascript" src="Http://Landly2/beer/Editor/jscripts/tiny_mce/tiny_mce.js"></script>
...
<div id="content">
	<div id="colOne">
        <h2>Slet link</h2>
            <p>På denne side kan du vælge et link som skal slettes, ved at klikke på det.</p>
            <p><strong>BEMÆRK!</strong> <br />Dette kan IKKE fortrydes, og du vil ikke blive bedt om at bekræfte sletningen.</p>
			<?php
			echo"<ul>";
			while($row = mysqli_fetch_assoc($links)) //En while der kører alle rækker igennem
			{
			echo "<LI><A href=\"../food/DB/DeleteLink_DB.php?LinkID=".$row['LinkID']."\">".$row['LinkText']."</a></LI>\n";
			}
			echo"</ul>";
			?>
	</div>
