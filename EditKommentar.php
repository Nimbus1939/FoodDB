...
$queryKommentar = mysqli_query($link, "Select * FROM Kommentar WHERE CommentID=".($_GET['CommentID'])." ORDER BY CreateDate DESC");
?>
...
<div id="content">
	<div id="colOne">
        <h2> Rediger kommentar</h2>
            <?php
				while($rowKommentar = mysqli_fetch_assoc($queryKommentar)) //En while der kører alle rækker igennem
					{
						echo "<div id=\"opret\">\n";
							echo "<form method=\"post\" action=\"../food/DB/UpdateKommentar_DB.php\">\n";
								echo "<p>\n";
									echo "<label><h3>Indhold</h3></label><br />\n";
           							echo "<input name=\"CommentID\" type=\"hidden\" value=\"".$_GET['CommentID']."\" />\n";
           							echo "<input name=\"OpskriftID\" type=\"hidden\" value=\"".$_GET['OpskriftID']."\" />\n";
									echo "<textarea name=\"Content\" cols=\"50\" rows=\"15\">".$rowKommentar['Content']."</textarea><br /> <br />\n";
