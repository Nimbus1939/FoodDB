...
<div id="content">
   <div id="colOne">
        <h2> Skift tilstand:</h2>
            <p> Ved at vælge herunder kan du skifte tilstand på databasen</p>
            <div id="opret">
                <form method="post" action="../food/DB/ChangeState_DB.php">
						<?php
                  $linkTilstand = mysqli_connect("localhost", "food", "Spark1Hund", "food"); //forbind til PROD databasen, for at hente tilstand.
                  if (mysqli_connect_errno())
                     ...
                     {
                        $state = mysqli_query($linkTilstand, "Select * FROM Tilstand");
                        while($rowsstate = mysqli_fetch_assoc($state)) //En while der kører alle State rækker igennem
								{
									if ($rowsstate['ActiveState'] == 1 AND $rowsstate['DBName'] == "food")
										{
											echo"<input type=\"radio\" name=\"Nr\" value=\"1\" checked=\"checked\"> PROD\n";
											echo"<input type=\"radio\" name=\"Nr\" value=\"2\"> DEV<br><br />\n";	
										}
									if ($rowsstate['ActiveState'] == 1 AND $rowsstate['DBName'] == "foodDev")
										{
											echo"<input type=\"radio\" name=\"Nr\" value=\"1\"> PROD\n";
											echo"<input type=\"radio\" name=\"Nr\" value=\"2\" checked=\"checked\"> DEV<br><br />\n";
										}
								}
                     }
						?>
                        <input type="submit" id="opret-submit" value="Skift" />
