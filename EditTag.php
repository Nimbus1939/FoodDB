...
$QqueryTag = "Select * FROM Tags WHERE TagID=".($_GET['TagID'])." ORDER BY CreateDate DESC";
...
<div id="content">
	<div id="colOne">
        <h2> Rediger tag</h2>
            <p> På denne side kan du redigere et tag</p>
            <?php
				while($rowTag = mysqli_fetch_assoc($queryTag)) //En while der kører alle rækker igennem
					{
						echo "<div id=\"opret\">\n";
							echo "<form method=\"post\" action=\"../food/DB/UpdateTag_DB.php\">\n";
								echo "<p>\n";
									echo "<label><h3>Navn</h3></label><br />\n";
									echo "<input name=\"TagName\" type=\"text\" size=\"50\" maxlength=\"50\" value=\"".$rowTag['TagName']."\"/> <br /><br />\n";
									echo "<label><h3>Indhold</h3></label><br />\n";
           							echo "<input name=\"TagID\" type=\"hidden\" value=\"".$_GET['TagID']."\" />\n";
									echo "<textarea name=\"Content\" cols=\"50\" rows=\"15\">".$rowTag['Content']."</textarea><br /> <br />\n";
