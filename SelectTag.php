...
$sider = mysqli_query($link, $link, $Qsider);
$typer = mysqli_query($link, $link, $Qtyper);
$kategori = mysqli_query($link, $link, $Qkategori);
$tags = mysqli_query($link, $link, $Qtags);
?>
...
<div id="content">
	<div id="colOne">
        <h2> Rediger tag</h2>
            <p>På denne side kan du vælge et tag som skal redigeres.</p>
            <p>Vælg et tag herunder, for at redigere det</p>
			<?php
			echo"<ul>";
			$query = mysqli_query($link, "Select * FROM Tags ORDER BY TagID DESC");
			while($row = mysqli_fetch_assoc($query)) //En while der kører alle rækker igennem
			{
			echo "<LI><A href=\"../food/EditTag.php?TagID=".$row['TagID']."\">".$row['TagName']."</a></LI>\n";
			}
			echo"</ul>";
			?>
	</div>
