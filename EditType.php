...
$QqueryType = "Select * FROM Type WHERE TypeID=".($_GET['TypeID'])." ORDER BY CreateDate DESC";
...
<div id="content">
	<div id="colOne">
        <h2> Rediger type</h2>
            <p> På denne side kan du redigere en type</p>
            <?php
				while($rowType = mysqli_fetch_assoc($queryType)) //En while der kører alle rækker igennem
					{
						echo "<div id=\"opret\">\n";
							echo "<form method=\"post\" action=\"../food/DB/UpdateType_DB.php\">\n";
								echo "<p>\n";
									echo "<label><h3>Navn</h3></label><br />\n";
									echo "<input name=\"TypeName\" type=\"text\" size=\"50\" maxlength=\"50\" value=\"".$rowType['TypeName']."\"/> <br /><br />\n";
									echo "<label><h3>Indhold</h3></label><br />\n";
           							echo "<input name=\"TypeID\" type=\"hidden\" value=\"".$_GET['TypeID']."\" />\n";
									echo "<textarea name=\"Content\" cols=\"50\" rows=\"15\">".$rowType['Content']."</textarea><br /> <br />\n";
